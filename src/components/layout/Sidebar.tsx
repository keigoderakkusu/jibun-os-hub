...
import {
  Layers, Cpu, Bot, Grid, ChevronLeft, ChevronRight,
  LayoutDashboard, Settings, Zap
} from 'lucide-react';

export type PageId = 'hub' | 'agent' | 'dashboard' | 'launcher';
...
const NAV_ITEMS: NavItem[] = [
  { id: 'dashboard', label: 'JIBUN-OS', icon: LayoutDashboard },
  { id: 'hub', label: 'OSS統合ハブ', icon: Cpu },
  { id: 'agent', label: 'AIエージェント', icon: Bot, badge: 'AI' },
  { id: 'launcher', label: 'マイシステム', icon: Grid },
];
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        {!collapsed && (
          <p className="px-2 mb-2 text-[10px] font-semibold uppercase tracking-widest" style={{ color: 'hsl(var(--sidebar-foreground))', opacity: 0.4 }}>
            メニュー
          </p>
        )}
        {NAV_ITEMS.map((item) => {
        ...
        {!collapsed && (
          <div className="flex items-center gap-2 px-2 py-2 mb-1">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-[10px] truncate" style={{ color: 'hsl(var(--sidebar-foreground))', opacity: 0.5 }}>Pro Plan</p>
            </div>
          </div>
        )}
        <button
