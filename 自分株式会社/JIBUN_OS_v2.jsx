...
export default function App() {
  ...
  const startProg = useCallback(() => {
    ...
    progRef.current = setInterval(() => {
      if (idx < PROG_STEPS.length) { setProgress(PROG_STEPS[idx][0]); setProgLabel(PROG_STEPS[idx][1]); idx++; }
    }, 900);
  }, []);
  ...
  const generate = useCallback(async () => {
    ...
    setLoading(true); setGenError(""); setArticle(null); setProgress(0); setProgLabel("Claude に接続中..."); startProg();
    ...
    try {
      ...
      if (!raw || raw.length < 50) throw new Error("生成結果が空です。APIキーのクレジット残高を確認してください。");
